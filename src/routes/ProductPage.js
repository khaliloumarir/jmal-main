    ...
    return (
        <div className="lg:px-24 sm:px-8">
            <div className="grid sm:grid-cols-2 grid-cols-1 ">
                <section>
                    <img src={`data:image/png;base64,${Buffer.from(currentMedia).toString("base64")}`} className="object-contain md:h-[500px]" />
                </section>
                    ...
                    <section>
                        ...
                        <ul className="grid sm:grid-cols-6 grid-cols-4  gap-2">
                            {currentProduct.Sizes?.split(",").map((size) => {
                                return <li className="bg-[#cecece] text-center  p-2"><p>{size}</p></li>
                            })}
                    ...
                    <section className="flex">
                        {currentProduct["Quantity"] &&
                            <>
                                ...
                                <p className="ml-2 text-[#5B6871] option">Available : {currentProduct["Quantity"]}</p>
                            </>
                        }
                        {
                            currentProduct["Minimum"] && <p className="ml-2 text-[#5B6871] option">Minimum : {currentProduct["Minimum"]}</p>
                        }

                    </section>
                    <section className="flex items-center">
                        <p className=" font-bold ">Price:</p>
                        <p className="ml-2  option">{validator.trim(currentProduct.Price)}</p>
                    </section>
                ...
                <Carousel containerClass="sm:col-span-2 my-2 "
                    itemClass="even:mx-2"
                    swipeable={true}
                    ...
                    }} src={`data:image/png;base64,${Buffer.from(currentProduct.image).toString("base64")}`} className="object-contain cursor-pointer" />
                    {otherMedia?.map((item) => {
                        return (
                            <img src={`data:image/png;base64,${Buffer.from(item.bytes).toString("base64")}`} onClick={() => {
...
function mapStateToProps(state) {
    console.log({ product: state.product, client: state.client, session: state.session })
    return { product: state.product, client: state.client, session: state.session }
}
